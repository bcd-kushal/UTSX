            ...
            <section className="flex flex-col items-stretch justify-start overflow-y-scroll h-[calc(100dvh_-_142px)] scrollbar-hide">
                    ...
                    <></> : <>
                        ...
                        <article className="grid grid-cols-3 gap-3">
                            {filteredCities.popularCities().map((city,index) => (
                                <div className="group flex flex-col items-center justify-center p-2 shadow-sm border-[0.667px] border-[#12121215] hover:border-[#ff385c85] hover:shadow-lg hover:text-[#ff385c] hover:bg-[#ff385c15] transition-all duration-500 cursor-pointer rounded-lg" onClick={handleLocationClick}>
                                    <span>svg</span>
                ...
                {filteredCities.allCities().length === 0 && filteredCities.popularCities().length===0 ? 
                    <div className="mt-16 flex flex-col items-center justify-center">
                        ...
                        <span className="text-[#3339] text-sm pt-1">Service unavailable for "{currLoc}"</span>
                    </div> : <></>
                } 
            </section>
        </div>
